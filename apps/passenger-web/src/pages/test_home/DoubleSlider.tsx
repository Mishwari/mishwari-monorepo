  ...
  return (
    <div className="px-2 select-none" dir="ltr">
      <div ref={trackRef} className="relative h-8" style={{ touchAction: 'none' }}>
        ...
        <div
          className="absolute h-1 bg-[#005687] rounded-full top-1/2 -translate-y-1/2"
          style={{ left: `${maxPosition}%`, width: `${minPosition - maxPosition}%` }}
        />
        <div
          className={`absolute w-4 h-4 bg-[#005687] rounded-full top-1/2 shadow-md z-10 ${isDragging === 'min' ? 'scale-125 cursor-grabbing' : 'cursor-grab hover:scale-110'} transition-transform`}
          style={{ left: `${minPosition}%`, transform: 'translate(-50%, -50%)' }}
          onMouseDown={handleMouseDown('min')}
        />
        <div
          className={`absolute w-4 h-4 bg-[#005687] rounded-full top-1/2 shadow-md z-10 ${isDragging === 'max' ? 'scale-125 cursor-grabbing' : 'cursor-grab hover:scale-110'} transition-transform`}
