...
function trip_details() {
  ...
  useEffect(() => {
    ...
    const fetchTripDetails = async () => {
      try {
        ...
        // If from_stop_id and to_stop_id are provided, filter to show only that segment
        if (from_stop_id && to_stop_id && data.stops) {
          const fromStop = data.stops.find((s: any) => s.id === Number(from_stop_id));
          const toStop = data.stops.find((s: any) => s.id === Number(to_stop_id));
          
          if (fromStop && toStop) {
            // Calculate segment-specific details
            const segmentData = {
              ...data,
  ...
  const handleGoToPayment = () => {
    const bookingDraft = {
      ...
      tripDetails,
      passengers: passengers.filter((p: Passenger) => p.is_checked),
      contactDetails,
      amount,
        ...
        <div className='px-2 sm:px-6 md:w-1/2 lg:max-w-xl'>
          <section className='mx-2 mt-4 p-4 bg-white shadow-lg text-brand-text-dark rounded-xl'>
            ...
            <div className='space-y-2'>
              ...
              {tripDetails?.trip_type === 'flexible' && tripDetails.departure_window_start && tripDetails.departure_window_end && (
                <div className='flex gap-2'>
                  <h1 className='font-bold'>نافذة المغادرة:</h1>
                  <h1>
                    {convertToReadableTime(tripDetails.departure_window_start)}
                    {' - '}
                    {convertToReadableTime(tripDetails.departure_window_end)}
                  </h1>
                </div>
              )}
          ...
          <section className='mx-2 h-max p-4 my-4 bg-white rounded-xl shadow-lg text-brand-text-dark'>
            ...
            <div className='mx-2 mt-4'>
              ...
              <div className='flex flex-wrap justify-start items-center gap-2 mt-4'>
                {tripDetails?.bus?.amenities && Object.entries(tripDetails.bus.amenities).map(([key, value], index) => (
                  value === 'true' && (
                    <div key={index} className='flex justify-center items-center text-center text-gray-600 text-xs font-bold gap-1.5 px-3 bg-gray-100 shadow rounded-2xl'>
                      <h1>{key === 'ac' ? 'مكيف' : key === 'wifi' ? 'واي فاي' : key}</h1>
              ...
              <div className='mt-6 space-y-2'>
                ...
                <div className='flex gap-2'>
                  <strong>السعة:</strong>
                  <p>{tripDetails?.bus?.capacity || 'غير محدد'} مقعد</p>
                </div>
                <div className='flex gap-2'>
                  <strong>المقاعد المتاحة:</strong>
                  <p>{tripDetails?.available_seats || 0} مقعد</p>
                </div>
                <div className='flex gap-2'>
