...
export default function CityDropdown({
  ...
  const filteredOptions = options.filter((option) => {
    ...
    // 1 char: match start of first word only
    if (normalizedQuery.length === 1) {
      return cityWords[0]?.startsWith(normalizedQuery);
    }
    
    // 2+ chars: match start of any word OR anywhere in city name
    return cityWords.some(word => word.startsWith(normalizedQuery)) || normalizedCity.includes(normalizedQuery);
  });
            ...
            {loading ? (
              ...
              filteredOptions.map((option) => (
                ...
                >
                  ...
                  {showTripCount && option.trip_count !== undefined && (
